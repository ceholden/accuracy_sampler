...
HOST=$(shell hostname)
ifeq ($(HOST),geo)
LOC=/project/earth/packages/accuracy_sampler
else ifeq ($(HOST),scc1)
LOC=/project/earth/packages/accuracy_sampler
else
LOC=$(HOME)/.qgis2/python/plugins
endif
# Override to home for now
LOC=$(HOME)/.qgis2/python/plugins
...
# translation
SOURCES = src/*.py
#TRANSLATIONS = i18n/accuracy_sampler_en.ts

# global
PLUGINNAME = accuracy_sampler

PY_FILES = src/*.py

EXTRAS = icon.png metadata.txt

UI_FILES = ui/ui_sampler.py
